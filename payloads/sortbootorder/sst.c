...
sst_enable_writing_status(struct spi_flash *flash)
{
	...
	if (ret)
		spi_debug("SF: Enabling Write Status failed\n");
	return ret;
}
...
sst_disable_writing(struct spi_flash *flash)
{
	...
	if (ret)
		spi_debug("SF: Disabling Write failed\n");
	return ret;
}
...
sst_byte_write(struct spi_flash *flash, u32 offset, const void *buf)
{
	...
	spi_debug("BP[%02x]: 0x%p => cmd = { 0x%02x 0x%06x }\n",
...
sst_write(struct spi_flash *flash, u32 offset, size_t len, const void *buf)
{
	...
	for (; actual < len - 1; actual += 2) {
		spi_debug("WP[%02x]: 0x%p => cmd = { 0x%02x 0x%06x }\n",
		...
		ret = spi_flash_cmd_write(flash->spi, cmd, cmd_len,
		                          buf + actual, 2);
		if (ret) {
			spi_debug("SF: SST word program failed\n");
			break;
		}
	...
	spi_debug("SF: SST: program %s %u bytes @ 0x%lx\n",
...
sst_unlock(struct spi_flash *flash)
{
	...
	if (ret)
		spi_debug("SF: Unable to set status byte\n");

	spi_debug("SF: SST: status = %x\n", spi_w8r8(flash->spi, CMD_SST_RDSR));
...
spi_flash_probe_sst(struct spi_slave *spi, u8 *idcode)
{
	...
	if (i == ARRAY_SIZE(sst_spi_flash_table)) {
		spi_debug("SF: Unsupported SST ID %02x\n", idcode[1]);
		return NULL;
	}
