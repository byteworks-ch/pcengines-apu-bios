...
#define MAX_ACPI_SIZE (45 * 1024)
	...
	if (high_table_pointer) {
		...
		/*
		 * Due to the way the tables are added to the ACPI table list, if not
		 * checked, random values of a random length can be added to the table.
		 * this can cause the table to wrap over the 4GB boundary.
		 * The new pointer should never be lower than the initial pointer.
		 */
		if (new_high_table_pointer < high_table_pointer) {
			printk(BIOS_ERR,
					"Error: One of the ACPI generation functions is reporting an incorrect size.");
		}
...
#endif
#if IS_ENABLED(CONFIG_NORTHBRIDGE_AMD_AGESA_FAMILY14) || IS_ENABLED(CONFIG_NORTHBRIDGE_AMD_AGESA_FAMILY15_TN) || IS_ENABLED(CONFIG_NORTHBRIDGE_AMD_AGESA_FAMILY16_KB)
...
#define MAX_COREBOOT_TABLE_SIZE (32 * 1024)
	...
	/* Write an additional bootorder file into cbmem for others to use */
	u32 cbmem_size = 0x400, total_size;
	struct bootorder_container *cbmem_ptr = NULL;
	struct bootorder_container tmpboot;
	char *dptr = NULL;

	get_mainboard_bootorder_table(&tmpboot, &dptr);
	if (dptr != NULL) {
		cbmem_ptr = cbmem_add (CBMEM_ID_BOOTORDER, cbmem_size);

		if (cbmem_ptr == NULL)
			die("Unable to write bootorder to CBMEM");

		cbmem_ptr->bootorder_signature = tmpboot.bootorder_signature;
		cbmem_ptr->bootorder_size = tmpboot.bootorder_size;
		memcpy(cbmem_ptr->bootorder_file_name, tmpboot.bootorder_file_name, sizeof(tmpboot.bootorder_file_name));
		memcpy(cbmem_ptr->bootorder_data, dptr, cbmem_ptr->bootorder_size);
		total_size = cbmem_ptr->bootorder_size + sizeof(struct bootorder_container);
		printk(BIOS_NOTICE, "bootorder file size %xh\n", cbmem_ptr->bootorder_size);
		printk(BIOS_NOTICE, "bootorder total size %xh\n", total_size);
		printk(BIOS_NOTICE, "bootorder struct in CBMEM\n");
		hexdump(BIOS_NOTICE, (void *)cbmem_ptr, total_size);
	}
#endif
