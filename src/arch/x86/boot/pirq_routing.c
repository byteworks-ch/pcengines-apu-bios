...
static void print_pirq_debug_info(struct irq_routing_table *rt){
	int i = 0;

	printk(BIOS_SPEW, "PIRQ Table: Interrupt Routing Table contents:\n"
			"\tSignature\t: %c%c%c%c\n"
			"\tVersion\t\t: 0x%x\n"
			"\tSize\t\t: 0x%x (%d) bytes\n"
			"\tRouter BDF\t: %02x:%02x.%x\n"
			"\tExclusive IRQs\t: 0x%x\n"
			"\tRouter PCI ID\t: 0x%x 0x%x\n"
			"\tMiniport Data\t: 0x%x\n"
			"\tChecksum\t: 0x%x\n"
			"\tMax PCI Slots\t: %d\n",
			rt->signature & 0xff, (rt->signature >> 8) & 0xff,
			(rt->signature >> 16) & 0xff, (rt->signature >> 24) & 0xff,
			rt->version, rt->size, rt->size, rt->rtr_bus, rt->rtr_devfn >> 3,
			rt->rtr_devfn & 0x7, rt->exclusive_irqs, rt->rtr_vendor,
			rt->rtr_device, rt->miniport_data, rt->checksum, CONFIG_IRQ_SLOT_COUNT);

	printk(BIOS_SPEW, "\t\t\t\t\tIRQ Link/Bitmap\n"
			"\tBus:Dev.Fn\tSlot\t INTA#\t INTB#\t INTC#\t INTD#\n");

	for (i = 0; i < CONFIG_IRQ_SLOT_COUNT; i++) {
		printk(BIOS_SPEW,
				"\t %02x:%02x.%x"	/* Bus:Dev.Fn */
				"\t %02x"			/* Slot Number */
				"\t%02x/%04x"		/* INTA# Link/Bitmap */
				"\t%02x/%04x"		/* INTB# Link/Bitmap */
				"\t%02x/%04x"		/* INTC# Link/Bitmap */
				"\t%02x/%04x\n",	/* INTD# Link/Bitmap */
				rt->slots[i].bus, rt->slots[i].devfn >> 3, rt->slots[i].devfn & 0x7, rt->slots[i].slot,
				rt->slots[i].irq[0].link, rt->slots[i].irq[0].bitmap,
				rt->slots[i].irq[1].link, rt->slots[i].irq[1].bitmap,
				rt->slots[i].irq[2].link, rt->slots[i].irq[2].bitmap,
				rt->slots[i].irq[3].link, rt->slots[i].irq[3].bitmap);
	}

	printk(BIOS_SPEW, "\nPIRQ Table: Hexdump of the PIRQ Table (0x%x bytes):", rt->size);
	hexdump(BIOS_SPEW, rt, rt->size);
}
...
static void check_pirq_routing_table(struct irq_routing_table *rt)
{
	...
	print_pirq_debug_info((struct irq_routing_table *)addr);
