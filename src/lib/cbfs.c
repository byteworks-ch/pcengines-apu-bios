...
void read_bootorder_from_cbfs( char *filename, struct bootorder_container *bootorder_ptr, char **data_ptr)
{
	...
	bootorder_ptr->bootorder_signature = CBMEM_ID_BOOTORDER;
	bootorder_ptr->bootorder_size = ntohl(cbfs_file->len);
	strcpy(bootorder_ptr->bootorder_file_name, filename);
	*data_ptr = CBFS_SUBHEADER(cbfs_file);
}
#endif /* CONFIG_MAINBOARD_BOOTORDER */
