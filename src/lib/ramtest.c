...
void not_so_quick_ram_check(void)
{
#define TEST_SIZE    64
#define LFSR_SEED    0xdeadbeef

	u32 rw_addr, cntr, lfsr, errs=0, saved[TEST_SIZE];
	volatile u32 *rw_ptr;
	u32    bot_mem = CONFIG_RAMBASE;
	msr_t  msr = rdmsr(TOP_MEM_MSR);
	u32    top_mem = msr.lo;

	printk(BIOS_NOTICE, "Testing memory from 0x%08x to 0x%08x\n", bot_mem, top_mem);

	/* save off the data at 0x10000000 */
	rw_addr = 0x10000000;
	for (cntr=0; cntr < TEST_SIZE; cntr+=4) {
		rw_ptr = (u32 *)(rw_addr + cntr);
		saved[cntr] = *rw_ptr;
	}

	lfsr = LFSR_SEED; // Set seed before writing
	for (rw_addr = bot_mem; rw_addr <= top_mem; rw_addr <<= 1) {
		if (!rw_addr) break;
		printk(BIOS_DEBUG, "Writing at 0x%08x\n", rw_addr);
		for (cntr=0; cntr < TEST_SIZE; cntr+=4) {
			rw_ptr = (u32 *)(rw_addr + cntr);
			get_lfsr( &lfsr);
			*rw_ptr = lfsr;
	...
	lfsr = LFSR_SEED; // Re-set seed before reading
	for (rw_addr = bot_mem; rw_addr <= top_mem; rw_addr <<= 1) {
		if (!rw_addr) break;
		printk(BIOS_DEBUG, "Reading at 0x%08x\n", rw_addr);
		for (cntr=0; cntr < TEST_SIZE; cntr+=4) {
			rw_ptr = (u32 *)(rw_addr + cntr);
			get_lfsr( &lfsr);
			if ( lfsr != *rw_ptr ) {
				printk(BIOS_ALERT, "Error:read %08x at 0x%08x should be %08x\n",
						*rw_ptr, rw_addr, lfsr);
				errs++;
			}
		}
	...
	/* restore the data at 0x10000000 */
	rw_addr = 0x10000000;
	for (cntr=0; cntr < TEST_SIZE; cntr+=4) {
		rw_ptr = (u32 *)(rw_addr + cntr);
		*rw_ptr =saved[cntr];
	...
	if (errs) printk(BIOS_ALERT, "Memory w/r/c errors = 0x%08x\n", errs);
}

static void get_lfsr( u32 *lfsr) {
u32 bit;
	bit   = ((*lfsr >> 0) ^ (*lfsr >> 29) ^ (*lfsr >> 31) ) & 1;
	*lfsr =  (*lfsr >> 1) | (bit << 31);
}
