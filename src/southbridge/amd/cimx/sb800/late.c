...
static void sb800_enable(device_t dev)
{
	...
	case SMBUS_DEVFN: /* 0:14h:0 SMBUS */
...
#error "The processor APIC IDs must be lifted to make room for the I/O APIC ID"
	...
	case IDE_DEVFN: /* 0:14h:1 IDE */
		printk(BIOS_DEBUG, "IDE: 0:14h.1\n");
		break;

	case HDA_DEVFN: /* 0:14h:2 HDA */
	...
	case SB_PCI_PORT_DEVFN: /* 0:14h:4 PCI */
		printk(BIOS_DEBUG, "PCI Bridge: 0:14h.4\n");
		if (!dev->enabled) {
			printk(BIOS_EMERG, "Error: PCIB (dev 0:14.4) should never be disabled!\n");
			printk(BIOS_EMERG, "  Hint: To disable the PCIB pins and use them as GPIOs set\n");
			printk(BIOS_EMERG, "  bit 0 of PM REG 0xEA.\n");
			die("Enable or remove device 0:14.4 in devicetree.cb\n");
		}
		break;
	...
	case SB_PCIE_PORT1_DEVFN: /* 0:15h:0 PCIe PortA */
