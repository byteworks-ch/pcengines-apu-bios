...
/* RTC date test error masks */
#define RTC_BCD_ERROR    0x01
#define RTC_RANGE_ERROR  0x02
...
static inline unsigned char cmos_read(unsigned char addr)
{
	...
	if (addr >= 128) {
		...
		addr |= inb(RTC_BASE_PORT) & NMI_DISABLE_BIT;
	}
...
static inline int test_bcd_encoding( u8 data )
{
	if ( ((data & 0x0F) > 0x09 ) || ((data & 0xF0) > 0x90 ) )
		return 1;
	return 0;
}

static inline u8 bcd_to_hex( u8 bcd_value )
{
	u8 hex_value = (((bcd_value & 0xF0)>>4) * 10) + (bcd_value & 0x0F);
	return hex_value;
}
