...
struct pci_irq_info {
	unsigned int	ioapic_irq_pin;	/* Pin on the IOAPIC to trigger */
	unsigned int	ioapic_src_pin;	/* IRQ from parent bus ((parent dev number << 2) | INT# (0 = INTA)) */
	unsigned int	ioapic_dst_id;	/* IOAPIC ID to send IRQ to */
	unsigned int    ioapic_flags;	/* IRQ Polarity and Trigger mode, see below */
	/*
	 * Polarity: 2 bits wide starting at bit 0:
	 * 00 = conforms to bus spec (PCI is active low)
	 * 01 = Active High
	 * 10 = Reserved
	 * 11 = Active Low
	 *
	 * Trigger mode: 2 bits wide starting at bit 2:
	 * 00 = conforms to bus spec (PCI is level triggered)
	 * 01 = Edge Triggered
	 * 10 = Reserved
	 * 11 = Level Triggered
	 */
